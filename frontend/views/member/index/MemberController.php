...
/*
    ...
    private function resendVerifEmail($user)
    {
        ...
        $message = "Kad patvirtintumėte registraciją spauskite šią nuorodą: 
                    <a href='".$url."'>http://pazintyslietuviams.co.uk/site/login?re=0&id=".$user->id."&ra=".$user->auth_key."</a>";
    ...
    public function actionUser()
    {
        ...
        if($post = Yii::$app->request->post()){
            if(isset($post['ajax']) && $post['ajax'] == 1){
                ...
                $user = Yii::$app->user->identity;
                $gavejoUser = User::find()->where(['id' => $post['otherID']])->one();

                if(time() - $gavejoUser->lastOnline > 600){//jeigu online

                    $alredyInformedAbout = explode(" ", $gavejoUser->msgEmailNot);

                    if(array_search($user->id, $alredyInformedAbout) === false){//jeigu dar neinformuotas apie zinute
                        Yii::$app->mailer->compose('_gavoteZinute', ['logo' => 'css/img/icons/logo2.jpg', 'avatars' => 'css/img/icons/avatarSectionEmail.jpg', 'link' => 'css/img/icons/link.jpg', 'user' => $user])
                        ->setFrom('pazintys@pazintyslietuviams.co.uk')
                        ->setTo($gavejoUser->email)
                        ->setSubject('Gavote asmeninę žinutę')
                        ->send();
                ...
                $chat->sender = Yii::$app->user->identity->id;
                $chat->reciever = $post['otherID'];
                $chat->message = $post['data'];
                $chat->newID =$post['otherID'];
                $chat->timestamp = time();
                $chat->save(false);
    ...
    public function actionGetusermodel()
    {
        ...
        $complete['miestas'] = $list[$info->miestas];
    ...
    public function actionHelp()
    {
        ...
        if($post = Yii::$app->request->post()){
            ...
            Yii::$app->mailer->compose()
                ->setFrom($user->email)
                ->setTo('pagalba@pazintyslietuviams.co.uk')
    ...
    public function actionChecktalpa()
    {
        ...
        if($user->valiuta >=  $kaina){
            ...
            $chat->sender = Yii::$app->user->id; 
            $chat->reciever = $id;
            $chat->message = "-%necd%% ".$user->username." jums padovanojo dovaną! <a href='".Url::to(['member/dovanaopen', 'id' => $model->id])."' style='border-bottom: 1px solid black;'>Išpakuoti</a>";
            $chat->dontShow = Yii::$app->user->id;
            $chat->timestamp = time();
            $chat->newID = $id;
            $chat->save(false);
    ...
    public function actionPayseraexecutepayment()
    {
        return $this->render('payseraexecutepayment');
    }
